    ...
    <div class="row">
      <div class="col-md-6">
        ...
        <table id="level1-table" class="table  table-bordered table-hover">
          ...
          <tbody>
          <tr v-for="category in level1" v-on:click="onClickLevel1(category)" v-bind:class="{'active':active.id===category.id}">
            <td>{{category.id}}</td>
            <td>{{category.name}}</td>
            <td>{{category.sort}}</td>
            <td>
        ...
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/business/admin/category/all').then((response)=>{
          ...
          for (var i=0; i<_this.categorys.length;i++){
            ...
            if (c.parent=='00000000'){
              _this.level1.push(c);
              for (var j=0;j<_this.categorys.length;j++){
...
<style scoped>
  .active td{
    background-color: #48d5ca !important;
  }
</style>